...
use clap_complete::generator::{utils, Generator};
...
_{bin_name_underscore}_commands() {{
    ...
    _describe -t commands '{bin_name} commands' commands \"$@\"
            ...
            subcommands_and_args =
                subcommands_of(parser_of(p, bin_name).expect("Error"))
        ));
    }
...
fn get_subcommands_of(parent: &Command) -> String {
    ...
    for &(ref name, ref bin_name) in &subcommand_names {
        ...
        let subcommand_args = get_args_of(
            parser_of(parent, &*bin_name).expect("Error"),
            Some(parent),
        );
        ...
        let children =
            get_subcommands_of(parser_of(parent, &*bin_name).expect("Error"));
...
fn get_args_of(parent: &Command, p_global: Option<&Command>) -> String {
    ...
    let mut segments =
        vec![String::from("_arguments \"${_arguments_options[@]}\" \\")];
    ...
    if parent.has_subcommands() {
        ...
        let subcommand_text =
            format!("\"*::: :->{name}\" \\", name = parent.get_name());
...
fn value_completion(arg: &Arg) -> Option<String> {
    if let Some(values) = clap_complete::generator::utils::possible_values(arg)
    {
        if values
            .iter()
    ...
    for arg in p.get_positionals() {
        ...
        let a = format!(
            ...
            help = arg
                ...
                .replace(':', "\\:"),
            value_completion =
                value_completion(arg).unwrap_or_else(|| "".to_string())
        );
