...
fn option_details_for_path(cmd: &Command, path: &str) -> String {
    ...
    let p = utils::find_subcommand_with_path(
        cmd,
...
fn vals_for(o: &Arg) -> String {
    ...
    if let Some(vals) = clap_complete::generator::utils::possible_values(o) {
        format!(
            "$(compgen -W \"{}\" -- \"${{cur}}\")",
            vals.iter()
